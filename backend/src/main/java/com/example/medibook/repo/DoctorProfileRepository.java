...
public interface DoctorProfileRepository extends JpaRepository<DoctorProfile, UUID> {
  ...
  @org.springframework.data.jpa.repository.Query("SELECT d FROM DoctorProfile d WHERE LOWER(d.user.fullName) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(d.specialty.name) LIKE LOWER(CONCAT('%', :q, '%'))")
